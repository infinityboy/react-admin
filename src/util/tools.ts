...
const tools = {
  ...
  */
  addMosaic(str: string | number): string {
    ...
    const lenth: number = s.length;
    const howmuch: number = (() => {
      if (s.length <= 2) {
        return s.length;
      }
      const l: number = s.length - 2;
      if (l <= 6) {
        return l;
      }
      return 6;
    })();
    const start: number = Math.floor((lenth - howmuch) / 2);
    const ret: string[] = s.split("").map((v, i) => {
  ...
  /** 正则 手机号验证 **/
  checkPhone(str: string | number) {
    const rex: RegExp = /^1[34578]\d{9}$/;
    return rex.test(String(str));
  },

  /** 正则 邮箱验证 **/
  checkEmail(str: string) {
    const rex: RegExp = /^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/;
    return rex.test(str);
  },
  /**
    字符串加密
  ...
  */
  ...
  clearNull(obj: IObjSimple): IObjSimple {
    const temp: IObjSimple = {};
